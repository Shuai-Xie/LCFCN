...
class BaseModel(nn.Module):
    ...
    def predict(self, batch, method="probs"):
        ...
        if method == "counts":
            ...
            return counts[None]  # add one dim

        elif method == "blobs":

            images = batch["images"].cuda()
            pred_mask = self(images).data.max(1)[1].squeeze().cpu().numpy()  # 2D, max=15

            h, w = pred_mask.shape
            blobs = np.zeros((self.n_classes - 1, h, w), int)

            for category_id in np.unique(pred_mask):  # [1-15]
                if category_id == 0:  # bg
                    continue
                blobs[category_id - 1] = morph.label(pred_mask == category_id)

            return blobs[None]  # add dim=1 on 1st dim, todo: train may need!
