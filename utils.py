...
def combine_image_blobs(image_raw, blobs_mask):
    # blobs_mask should be 2D, then cvt class_val to class_color
    blobs_rgb = label2rgb(blobs_mask)

    image_raw = image_raw * 0.5 + blobs_rgb * 0.5
    image_raw /= image_raw.max()

    if len(blobs_mask.shape) == 2:
        image_raw = mark_boundaries(image_raw, label_img=blobs_mask)
    else:
        for idx in range(blobs_mask.shape[0]):  # traverse each class
            image_raw = mark_boundaries(image_raw, label_img=blobs_mask[idx])  # label_img must be 2D int
